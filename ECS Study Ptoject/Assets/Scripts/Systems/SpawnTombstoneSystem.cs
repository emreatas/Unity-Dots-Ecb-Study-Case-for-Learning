...
public partial struct SpawnTombstoneSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        var graveyard = SystemAPI.GetAspect<GraveyardAcpect>(graveyardEntity);

        var ecb = new EntityCommandBuffer(Allocator.Temp);
        var tombstoneOffset = new float3(0f, -2f, 1f);

        var builder = new BlobBuilder(Allocator.Temp);
        ref var spawnPoint = ref builder.ConstructRoot<ZombieSpawnPointBlob>();
        var arrayBuilder = builder.Allocate(ref spawnPoint.Value, graveyard.NumberTombstonesToSpawn);
